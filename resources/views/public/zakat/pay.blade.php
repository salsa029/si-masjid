...
@section('content')
    <div class="mx-auto max-w-md text-center">
        ...
        <div class="rounded-xl border border-gray-100 bg-white p-8 shadow-sm">
            <p class="text-sm text-gray-500">Zakat — {{ $zakat->zakatType->name }}</p>
            <p class="mt-2 text-3xl font-bold text-emerald-700">Rp {{ number_format($zakat->amount, 0, ',', '.') }}</p>
            <p class="mt-1 text-xs text-gray-400">No. Transaksi: {{ $zakat->transaction_number }}</p>
            ...
            @if ($zakat->payment_status === 'pending' && $zakat->payment_method === 'midtrans')
                ...
                <a href="{{ route('public.zakat.check-status', $zakat) }}"
                    class="mt-3 block text-xs text-gray-400 underline">
                    Sudah bayar? Cek Status Pembayaran
                </a>
            @elseif ($zakat->payment_status === 'pending' && $zakat->payment_method === 'manual_transfer')
                <div class="mt-6 space-y-1 rounded-lg bg-gray-50 p-4 text-left text-sm">
                    ...
                    <p class="text-xs text-amber-600">Batas waktu:
                        {{ $zakat->reserved_until?->translatedFormat('d F Y, H:i') }} WIB</p>
                </div>

                <form method="POST" action="{{ route('public.zakat.upload-proof', $zakat) }}" enctype="multipart/form-data"
            ...
            @elseif ($zakat->payment_status === 'awaiting_verification')
            ...
            @elseif ($zakat->payment_status === 'success')
                <div class="mt-6 rounded-lg bg-emerald-50 p-4 text-sm text-emerald-700">
                    Pembayaran berhasil. Terima kasih atas zakat Anda.
                </div>
            ...
            @else
                <div class="mt-6 rounded-lg bg-red-50 p-4 text-sm text-red-700">
                    Transaksi {{ $zakat->payment_status === 'expired' ? 'telah kedaluwarsa' : 'gagal diproses' }}.
                    @if ($zakat->verification_note)
                        <br><span class="text-xs">Catatan Admin: {{ $zakat->verification_note }}</span>
                    @endif
                </div>
            @endif

            <a href="{{ route('public.zakat.history') }}" class="mt-4 block text-xs text-gray-400 underline">
                Lihat Riwayat Zakat Saya
            </a>
        </div>
    ...
    @if ($snapToken)
        ...
        <script>
            document.getElementById('pay-button').addEventListener('click', function() {
                snap.pay('{{ $snapToken }}', {
                    onSuccess: function() {
                        window.location.href = '{{ route('public.zakat.check-status', $zakat) }}';
                    },
                    onPending: function() {
                        window.location.href = '{{ route('public.zakat.check-status', $zakat) }}';
                    },
